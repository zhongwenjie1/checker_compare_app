# .venv312/lib/python3.12/site-packages/pandas/tests/window/test_api.py
...
def test_centered_axis_validation(step):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        df.rolling(window=3, center=True, axis=0, step=step).me
```
