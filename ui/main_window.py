...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.lbl_std_ok = self._make_chip("标准已匹配: 0", "#E8F5E9")
        self.lbl_std_ng = self._make_chip("标准未匹配: 0", "#FFF3E0")
        self.lbl_sys_ok = self._make_chip("系统已匹配: 0", "#E8F5E9")
        self.lbl_sys_ng = self._make_chip("系统未匹配: 0", "#FFF3E0")
    ...
    def _update_summary_chips(self, ok_std: int, ng_std: int, ok_sys: int, ng_sys: int):
        self.lbl_std_ok.setText(f"标准已匹配: {ok_std}")
        self.lbl_std_ng.setText(f"标准未匹配: {ng_std}")
        self.lbl_sys_ok.setText(f"系统已匹配: {ok_sys}")
        self.lbl_sys_ng.setText(f"系统未匹配: {ng_sys}")
    ...
    def _on_std_loaded(self, df):
        ...
        if not self._sized_std_once:
            QTimer.singleShot(100, lambda: self._autosize_columns_fast(self.table_std))
    ...
    def _on_sys_loaded(self, df):
        ...
        if not self._sized_sys_once:
            QTimer.singleShot(100, lambda: self._autosize_columns_fast(self.table_sys))
    ...
    def _on_compared(self, result):
        ...
        # 统一表格里的比对结果文案：OK/NG → 已匹配/未匹配
        try:
            std_df["比对结果"] = std_df["比对结果"].replace({
                "OK": "已匹配",
                "NG": "未匹配",
            })
            sys_df["比对结果"] = sys_df["比对结果"].replace({
                "OK": "已匹配",
                "NG": "未匹配",
            })
        except Exception:
        ...
        try:
            ok_std = (std_df["比对结果"] == "已匹配").sum()
            ng_std = (std_df["比对结果"] == "未匹配").sum()
            ok_sys = (sys_df["比对结果"] == "已匹配").sum()
            ng_sys = (sys_df["比对结果"] == "未匹配").sum()
        except Exception:
